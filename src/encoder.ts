...
export function encodeElements(
  ...
  if (!values || (isArray && values.length === 0) || (!isArray && Object.keys(values).length === 0)) {
    if (context.length >= 3) {
  ...
  while (elementIndex < elements.length) {
    ...
    if (isArray) {
      ...
      if (element.maxCount > 1) {
        textValues.push(
          values[elementIndex]?.map((value: any) => element.encode(value, context, version)).join(separator)
        );
      } else {
      ...
      if (element.maxCount > 1) {
        textValues.push(
          values[element.name]?.map((value: any) => element.encode(value, context, version)).join(separator)
        );
      } else {
