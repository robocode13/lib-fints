...
export class Message {
  ...
  findAllUnknownSegments(segmentId: string): UnknownSegment[] {
    const unknownSegments = this.segments.filter((segment) => segment.header.segId === UnkownId) as UnknownSegment[];
  ...
  getBankAnswers(): BankAnswer[] {
    ...
    hirmgSegments.forEach((hirmg) =>
      bankAnswers.push(
        ...hirmg.answers.map((message) => ({ code: message.code, text: message.text, params: message.params }))
      )
    );
    ...
    hirmsSegments.forEach((hirms) =>
      bankAnswers.push(
        ...hirms.answers.map((message) => ({ code: message.code, text: message.text, params: message.params }))
      )
    );
...
export class CustomerMessage extends Message {
  ...
  encode(): string {
    ...
    if (this.lastSignatureNumber > 0) {
      ...
      const hnvsk: HNVSKSegment = {
        ...
        encryption: { use: 2, mode: 2, algorithm: 13, keyParamValue: '00000000', keyParamName: 5, initParamName: 1 },
        key: {
  ...
  sign(
    ...
    const hnshk: HNSHKSegment = {
      ...
      key: { bank: { country: countryCode, bankId: bankId }, userId: userId, keyType: 'S', keyNr: 0, keyVersion: 0 },
    };
...
export class CustomerOrderMessage extends CustomerMessage {
  constructor(public orderSegId: string, public orderResponseSegId: string, dialogId: string = '0', msgNr = 1) {
